...
type Client struct {
	...
	closing  bool // user call Close
	shutdown bool // server told us to stop
}
...
func dialTimeout(f newClientFunc, network, address string, opts ...*Option) (client *Client, err error) {
	...
	case <-time.After(opt.ConnectionTimeout):
		return nil, fmt.Errorf("rpc client: connect timeout, expect within %s seconds", opt.ConnectionTimeout)
	case result := <-ch:
		return result.client, result.err
	}
